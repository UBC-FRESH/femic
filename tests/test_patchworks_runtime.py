...
def test_run_patchworks_headless_pin_updates_manifest_and_stage_outputs(
    ...
    assert (
        manifest["inputs"]["scenario_target"] == "flow.even.product.Yield.managed.Total"
    )
...
def test_run_patchworks_headless_pin_defaults_stage_under_log_dir(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    cfg_path = _write_runtime_config(tmp_path)
    cfg = load_patchworks_runtime_config(cfg_path)
    ...
    expected_stage_dir = (tmp_path / "logs/headless_stage/headless_default").resolve()
    ...
    def _fake_run_patchworks_beanshell_script(
        *, script_path: Path, script_args: tuple[str, ...], **_kwargs
        ...
        script_text = script_path.read_text(encoding="utf-8")
        assert json.dumps(str(pin_path.resolve())) in script_text
        assert json.dumps(expected_stage_dir.as_posix()) in script_text
        expected_stage_dir.mkdir(parents=True, exist_ok=True)
        (expected_stage_dir / "summary.csv").write_text("ok\n", encoding="utf-8")
        manifest_path = (
            tmp_path / "logs/patchworks_beanshell_manifest-headless_default.json"
        )
        manifest_path.parent.mkdir(parents=True, exist_ok=True)
        manifest_path.write_text(
            json.dumps(
                {
                    "run_id": "headless_default",
                    "returncode": 0,
                    "failures": [],
                }
            ),
    ...
    assert result.returncode == 0
    assert result.stage_dir == expected_stage_dir
    manifest = json.loads(result.manifest_path.read_text(encoding="utf-8"))
    assert manifest["inputs"]["stage_label"] == expected_stage_dir.as_posix()
    assert manifest["outputs"]["stage_dir"] == str(expected_stage_dir)
    assert manifest["outputs"]["saved_file_count"] == 1
...
def test_run_patchworks_headless_pin_resolves_default_scenario_target(
    ...
    def _fake_run_patchworks_beanshell_script(
        *, script_path: Path, script_args: tuple[str, ...], **_kwargs
        ...
        (stage_dir / "summary.csv").write_text("ok\n", encoding="utf-8")
        manifest_path = (
            tmp_path / "logs/patchworks_beanshell_manifest-default_target.json"
        )
        manifest_path.parent.mkdir(parents=True, exist_ok=True)
        manifest_path.write_text(
            json.dumps(
