...
def test_resolve_legacy_siteprod_artifacts_prefers_instance_local_pair(tmp_path: Path) -> None:
    instance_data_root = tmp_path / "instance_data"
    instance_data_root.mkdir(parents=True)
    ...
    (external_root / "bc" / "siteprod" / "siteprod.bandmap.json").write_text("{}", encoding="utf-8")
    ...
    assert resolved.siteprod_bandmap_path == instance_data_root / "siteprod.bandmap.json"


def test_resolve_legacy_siteprod_artifacts_falls_back_to_external_pair(tmp_path: Path) -> None:
    instance_data_root = tmp_path / "instance_data"
    instance_data_root.mkdir(parents=True)
    external_root = tmp_path / "external_data"
    (external_root / "bc" / "siteprod").mkdir(parents=True)
...
def test_resolve_legacy_siteprod_artifacts_returns_legacy_paths_when_pair_missing(tmp_path: Path) -> None:
    instance_data_root = tmp_path / "instance_data"
    instance_data_root.mkdir(parents=True)
    external_root = tmp_path / "external_data"
    external_root.mkdir(parents=True)
