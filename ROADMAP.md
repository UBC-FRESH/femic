...
- 2026-03-22 (P22.9e recovery checkpoint): recovered the missing `ctfert` asset surface on a dedicated recovery branch, added config-level `retention.default_retention_fraction` support in FEMIC, created `config/silviculture.k3z.baseline.yaml`, and aligned the K3Z `ctfert` / `pctct` silviculture configs to the baseline retention policy (`0.05` default, `1.0` for `CWHvm_CW+YC` and `CWHvm_CW+PLC`). Matrix Builder rebuilds the recovered `ctfert` surface with non-zero passive area again (`Managed 1622.3092129471195`, `Passive 159.00402311063843` in `patchworks_matrixbuilder_stderr-k3z_ctfert_recovery_smoke_20260321_c.log`). A narrower canonical-path test on 2026-03-22 successfully reconstructed an AU-bearing `data/ria_vri_vclr1p_checkpoint7.feather` from the existing K3Z `checkpoint1` + bundle/TIPSY artifacts and confirmed that `femic export patchworks` now runs mechanically against that checkpoint. The remaining reason `P22.9e` stays open is no longer a missing artifact: canonical export currently rebuilds the broader raw K3Z footprint (`au=27`, `fragments=260`) instead of the intended teaching baseline footprint (`au=14`, `fragments=218`). The unresolved work is now to reconcile that canonical checkpoint7/export path with the baseline teaching-landbase mask, not simply to generate checkpoint7 at all.
