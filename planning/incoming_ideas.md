...
[feature] After P98 lands the narrower FreshForge model-instance materialization workflow, revisit the broader optional FEMIC-wide "DataLad wrapper mode" for tracking the full lifecycle of data transformations with `datalad run`: raw source acquisition, model-input publication, scenario execution, scenario outputs, and post-hoc analysis/reporting. Keep this broader provenance/lifecycle mode separate from the P98 bootstrap/materialization workflow now tracked in ROADMAP.md and issue `#270`.
