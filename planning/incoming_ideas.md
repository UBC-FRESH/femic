...
[feature] I want to do a deep refactor of the FEMIC workflow and pipeline, to move away from the current "legacy monolithic python scripts" (i.e., `00_data-prep.py`, `01a_run-tsa.py`, `01b_run-tsa.py`) and towards the following concept: FEMIC runs a name "pipeline" (from a registry of standard system pipelines, and possibly pulling from a public user-contributed pipeline GitHub repository, or optionally from user-defined local pipeline registry), where each pipeline is basically just an sequence of named "recipes" (with optional user-overlays and user-overrides). New pipelines can be created by mixing and matching recipes from standard/public-user-contributed/local-user-defined recipes and user-overlays. That is all. Probably a bit complex to implement, but super simple conceptually (to explain to a new or advanced user), and I think almost infinitely flexible in terms of defining "what does FEMIC do?". Model instances are fully reproducible because you just need to pull on the "runbook" for a given instance to fully rebuild it from scratch (or, by default, from locally stored artifacts if detected and available), where the runbook can be as simple as "execute standard system pipeline X, with default parameters" or as complex as "run the following full custom sequence of custom recipes plus local user overlays that point to fully non-standard data sources". This is basically fully consistent with the recipe-based workflows we already started implementing in GitHub parent issue 122, so should not disrupt or invalidate recent work. Recipes at the far downstream end of the pipeline could include "implement a commercial thinning treatment option the target Patchworks model instance we are building", or "generate a set of standard scenarios to run on our Patchworks instance, and bundle these as a scenario-set unit", or "deploy a standard set of Patchworks scenrio output analysis logical operations and compile a standard scenario-set comparison and analysis report, e.g., to build a trade-off curve between maximum even-flow harvest volume and various intensities of a given biodiversity conservation policy", etc. Recipes can also include for example be used to pull one of the "banks" of BTC-derived TIPSY indicator raw outputs and inject it into a target Patchworks model instance (complete with logic for how to weave this into the existing ForestModel XML file as curve and attribute and attributeS elements without breaking the stuff that is already there, compiling the tracks with the matrix building, and doing the final wiring of the raw attributes from compiled tracks into functional Patchworks accounts with sensible names that do not break the existing account-naming system, aggregate accounts, custom Patchworks reports, and weaving the new targets into new scenario-definition YAML files so the new bank of indicators can easily and immediately be user-validated and deployed in the analysis). 
