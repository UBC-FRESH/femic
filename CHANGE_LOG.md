...
- Focused validation:
  ...
  - `.\.venv\Scripts\python.exe -m mypy src`
## 2026-04-19 - Rebuilt `tsr.thlb_strict` to execute only locked validated step logic
- `#169` bounded strict-runner slice:
...
- Focused validation:
  ...
  - `.\.venv\Scripts\python.exe -m ruff check src/femic/tsr_catalog/recipes.py tests/test_tsr_recipes.py`
  - `.\.venv\Scripts\python.exe -m mypy src`
## 2026-04-19 - Made strict LU cache reuse schema-aware
- `#169` bounded strict-runner fix:
  - updated the strict locked-step executor to pass `expected_columns` into `_load_cached_landscape_unit_partition_records(...)` so cache reuse now requires an exact match with the current checkpoint schema, including strict-state columns like `_row_id`, `_stand_area_sqm`, `thlb_fact`, and `thlb`; and
  - added a focused regression test proving strict execution supplies the current checkpoint column set to cache lookup instead of reusing stale LU chunk caches on row count + area alone.
- Focused validation:
...
## 2026-04-28 - Planned Phase 56 MKRF rebuild gap closure
- `#172` planning-only slice:
  - added `Phase 56` to `ROADMAP.md` as the ordered post-P55 rebuild gap-closure sequence;
  - converted the P55.19 no-go blockers into bounded tasks for generated XML reconciliation, pointer-only compiled track-table resolution, builder activation design, source-input publication boundaries, and rebuild-readiness criteria; and
  - updated `planning/mkrf_legacy_decompile.md` so `P56.1` is the active next bounded move.
- Scope boundary preserved:
  - no generated XML, upstream mapping data, roads, outputs, direct workbook payload, or new raw/source artifacts were ingested;
  - no curve, retention, attribute, stratum, or full XML builder behavior was activated; and
