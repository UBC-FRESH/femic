...
- Exposed `ModelBuildSpec` and `WorkspaceManifest` from the FEMIC package root
  and added `load_model_build_spec()` for Coordinator-submitted YAML/JSON
  requests, with focused file-backed coverage.
...
## 2026-08-02
- Planned P110.2: register the six FEMIC model-build provider contracts used by
  the P110.1 FreshForge graph.
  - The detailed plan is recorded in
    `planning/phase110_next_steps_provider_registration.md`.
  - The next slice will reconcile the graph's provider namespace with
    FreshForge parsing, resolve provider-aware metadata, and preserve the
    non-executing boundary. It will not claim stage execution or populate a
    workspace manifest.
- Implemented the non-executing FEMIC P110.1 workflow compiler.
  - Added `src/femic/model_workflow.py` with `plan_model_from_spec()` and the
    serializable `ModelWorkflowPlan` result.
  - Added a deterministic six-stage FreshForge graph for inventory,
    source-preflight, typed compile/emit, dataset lint/import, structural
    verification, and package/evidence.
  - Preserved FreshForge structural validation and provider-aware planning
    diagnostics. The current unregistered `femic.model_build.*` contracts are
    reported as unavailable and produce no executable plan nodes.
  - Added focused tests for stable identity, graph dependencies, deterministic
    serialization, invalid requests, unsupported engines, and non-mutation.
  - Verified formatting/lint and 37 focused FEMIC tests passing. FreshForge's
    separate fixture tests require its local fixture provider entry point to be
    installed; the failure is environmental and outside this FEMIC change.
- Completed the initial FEMIC Phase 110 construction and dispatch slice.
  - Added a FEMIC-owned ws3 adapter that converts the four canonical Woodstock
    CSV tables into a typed `ModelSpec`, emits deterministic sections through
    `ModelBuilder`, and returns a fresh imported ws3 model.
  - Replaced the builtin ws3 smoke path's manual section imports with the typed
    adapter while preserving the legacy CSV bridge for parity comparison.
  - Added Coordinator-facing `build_model_from_spec()` dispatch with
    `propose`, `dry_run`, `apply`, and `run` approval modes. Proposal mode is
    non-mutating; dry-run writes to scratch; apply/run write only to the
    declared output root.
  - Added focused adapter and dispatch coverage, including unsupported engine
    handling, validation plans, output declarations, fresh-model assertions,
    and import-boundary behavior.
  - Verified the Phase 110 construction slice with 32 focused tests passing.
- Opened Phase 110 task `P110.1` as FEMIC issue `#306` and linked it under
  parent issue `#305`.
  - The next implementation slice is FreshForge workflow compilation: turn a
    validated `ModelBuildSpec` into a deterministic `WorkflowSpec` and
    provider-aware non-executing `RunPlan`.
  - The plan explicitly preserves package ownership: FEMIC compiles the
    domain request, FreshForge validates/plans/executes workflow graphs, and
    ws3 owns forest-estate contracts and verification.
  - Planning surface: `planning/phase110_next_steps_workflow_compilation.md`.
...
- Focused validation:
  ...
  - `.\.venv\Scripts\python.exe -m pytest tests/test_cli_main.py -k "pipelines_run" -q`
  - `.\.venv\Scripts\python.exe -m ruff check src/femic/named_pipelines.py tests/test_named_pipe